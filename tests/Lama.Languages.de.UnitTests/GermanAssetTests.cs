...
namespace Lama.Languages.de.UnitTests;

/// <summary>
/// Validates that the German language pack embedded assets are well-formed
/// and contain the expected data structure.
/// </summary>
public class GermanAssetTests
{
    private static readonly Assembly Assembly = System.Reflection.Assembly.Load("Lama.Languages.de");
    ...
    {
        var act = () => LoadEmbeddedResource("Lama.Languages.de.assets.scores.json");
        act.Should().NotThrow();
    }
    ...
    {
        var act = () => LoadEmbeddedResource("Lama.Languages.de.assets.tile-distribution.json");
        act.Should().NotThrow();
    }
    ...
    {
        using var doc = LoadEmbeddedResource("Lama.Languages.de.assets.scores.json");
        var scores = doc.RootElement.GetProperty("scores");

        for (var c = 'A'; c <= 'Z'; c++)
        {
            scores.TryGetProperty(c.ToString(), out var score).Should().BeTrue(
                $"letter '{c}' should have a score in German scores.json");
    ...
    [Fact]
    public void ScoresJson_GermanSpecificValues_AreCorrect()
    {
        using var doc = LoadEmbeddedResource("Lama.Languages.de.assets.scores.json");
        var scores = doc.RootElement.GetProperty("scores");

        // Known German Scrabble values
        scores.GetProperty("A").GetInt32().Should().Be(1);
        scores.GetProperty("Q").GetInt32().Should().Be(10);
        scores.GetProperty("Y").GetInt32().Should().Be(10);
    }

    [Fact]
    public void TileDistributionJson_HasBaseDistribution()
    {
        using var doc = LoadEmbeddedResource("Lama.Languages.de.assets.tile-distribution.json");
    ...
    public void TileDistributionJson_ContainsAll26Letters()
    {
        ...
        for (var c = 'A'; c <= 'Z'; c++)
        {
            baseDist.TryGetProperty(c.ToString(), out var count).Should().BeTrue(
                $"letter '{c}' should be in German tile distribution");
    ...
    public void TileDistributionJson_Total_IsReasonable()
    {
        ...
        total.Should().BeGreaterThan(80, "German Scrabble has ~119 tiles");
