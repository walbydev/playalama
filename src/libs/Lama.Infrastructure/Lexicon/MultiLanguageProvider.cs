...
/// Lettres/scores/distribution proviennent de la langue primaire.
/// </summary>
public sealed class MultiLanguageProvider : IGameLanguageProvider
{
    ...
    public MultiLanguageProvider(IReadOnlyList<IGameLanguageProvider> providers)
    {
        ...
        _union = providers.Count == 1 ? providers[0].GetDictionary() : new UnionWordSet(providers.Select(p => p.GetDictionary()).ToList());
        _name = string.Join(" + ", providers.Select(p => p.GetLanguageName()));
    }

    public IReadOnlySet<string> GetDictionary() => _union;
    public IReadOnlyDictionary<char, int> GetLetterScores() => _primary.GetLetterScores();
