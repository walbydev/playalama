...
- **Versioning and build**: `.build-info` (JSON) sync via script to `BuildInfoConstants.cs`; make targets: `build` (auto-increments build #), `release VERSION=x.y.z` (sets version), `build-generate` (regenerates timestamp). Build info is shown in the footer (`Footer.razor`) and a sticky bottom bar on the game page (`GameBuildBar.razor`), both using the static class without HTTP.
